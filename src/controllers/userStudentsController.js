...
export const getUserStudent = async (req,res) => {
    try {
        const result = await db.query(`SELECT users.email, Estudiantes.name, Estudiantes.cohort, Estudiantes.age, Estudiantes.phone, Estudiantes.status, Estudiantes.gender, programs.name as programa FROM Estudiantes, users, programs WHERE Estudiantes.id_user = users.id and Estudiantes.id_program = programs.id;`)
        res.json(result);
    } catch (error) {
